...
public class StringUtil {
    ...
    public static void list2Map() {
        emojiMap.put("[ling]", R.mipmap.ling);
        emojiMap.put("[yi]", R.mipmap.yi);
        emojiMap.put("[er]", R.mipmap.er);
        emojiMap.put("[san]", R.mipmap.san);
        emojiMap.put("[si]", R.mipmap.si);
        emojiMap.put("[wu]", R.mipmap.wu);
        emojiMap.put("[liu]", R.mipmap.liu);
        emojiMap.put("[qi]", R.mipmap.qi);
        emojiMap.put("[ba]", R.mipmap.ba);
        emojiMap.put("[jiu]", R.mipmap.jiu);
        emojiMap.put("[c]", R.mipmap.c);
        emojiMap.put("[r]", R.mipmap.r);
        emojiMap.put("[xiaochou]", R.mipmap.xiaochou);
     ...
     * @return
     */
    public static ArrayList<EmojiBean> getList() {
        ArrayList<EmojiBean> list = new ArrayList<>();
        Set<String> set = emojiMap.keySet();
        Iterator<String> it = set.iterator();
        while (it.hasNext()) {
            String name = it.next();
            list.add(new EmojiBean(name, emojiMap.get(name)));
        }
        return list;
    }
